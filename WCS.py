...
class WorstCaseStackParser(object):
    def __init__(
        ...
        self.dir = dir  # Working directory
        self.su_ext = ".su"
        self.obj_ext = ".o"
        self.manual_ext = ".msu"
        self.read_elf_path = "readelf"  # You may need to enter the full path here
        self.stdout_encoding = "utf-8"  # System dependant
    ...
    def read_obj(self, tu):
        ...
        """
        symbols = self.read_symbols(tu[0 : tu.rindex(".")] + self.obj_ext)
    ...
    def read_rtl(self, tu):
        ...
        for line_ in open(tu + rtl_ext).readlines():
            ...
            if m:
                fxn_name = m.group(2)
                fxn_dict2 = find_fxn(tu, fxn_name, self.call_graph)
                if not fxn_dict2:
                    raise Exception(
    ...
    def read_su(self, tu):
        ...
        # su_line = re.compile(r'^([^ :]+):([\d]+):([\d]+):([a-zA-Z]+)\s([a-zA-Z_]+)\t(\d+)\t(\S+)$')
        su_line = re.compile(
            r"([^\n:]+):([\d]+):([\d]+):([\w\d\s\*_]+)\s+([\w\d_]+)\(.*\)\t+(\d+)\t"
        )
        i = 1

        for line in open(tu[0 : tu.rindex(".")] + self.su_ext).readlines():
            m = su_line.match(line)
            if m:
                fxn = m.group(5)
                fxn_dict2 = find_demangled_fxn(tu, fxn, self.call_graph)
    ...
    def resolve_all_calls(self):
        def resolve_calls(fxn_dict2):
            ...
            for call in fxn_dict2["calls"]:
                call_dict = find_fxn(fxn_dict2["tu"], call, self.call_graph)
                if call_dict:
    ...
    def find_rtl_ext(self):
        ...
        for root, directories, filenames in os.walk(self.dir):
            for f in filenames:
                if f.endswith(self.rtl_ext_end):
                    rtl_ext = f[f[: -len(self.rtl_ext_end)].rindex(".") :]
    ...
    def find_files(self, dot_c_included=False):
        ...
        for f in files:
            ...
            base_to_use = base if dot_c_included else short_base
            print(base_to_use)
            if (
                ...
                print(
                    "Reading: {}{}, {}{}, {}{}".format(
                        base, rtl_ext, short_base, self.su_ext, short_base, self.obj_ext
                    )
                )
...
def main(argv):
    ...
    tu_list, manual_list = find_files(args.dot_c_included)
    ...
    for tu in tu_list:
        wcs_parser.read_rtl(tu, call_graph)
    for tu in tu_list:
        wcs_parser.read_su(tu, call_graph)

    # Read manual files
    for m in manual_list:
        wcs_parser.read_manual(m, call_graph)
